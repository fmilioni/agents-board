...
The review runs in a fresh read-only subagent dispatched by `implement`. Codex explicitly loads the internal **`reviewer`** skill; Claude Code dispatches its packaged **`agents-board:reviewer`** agent. Neither is a normal user-facing workflow entry point. The "never assume — resolve open decisions" doctrine is carried **inline** in `plan` and `implement`, so each user-facing skill is self-contained.
