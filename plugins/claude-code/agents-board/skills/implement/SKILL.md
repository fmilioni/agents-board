...
Spawn the **`agents-board:reviewer`** agent (`Agent` tool) — it's read-only by construction (no board tools, no Edit/Write), so it reads code + git and reports; **you** dispose of its findings. You have board access, so curate into its prompt everything it needs:
