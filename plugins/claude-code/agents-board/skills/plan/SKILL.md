...
Turn a demand into well-formed work through dialogue with the user, then materialize it as **cards in agents-board**. The artifact is the cards in the MCP — not a spec file. Execution happens afterwards, card by card, via the **`implement`** skill.
...
- **Cross-reference by the real key, in full** — `AB-53, AB-54` (auto-links), never a range `AB-53/54` or a positional alias.
