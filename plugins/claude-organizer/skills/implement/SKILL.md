...
2. **Agree the git flow** for the run — unless `AGENTS.md` already defines one (or a legacy `CLAUDE.md` does), in which case follow it and do not ask. In batch mode the options matter most: one PR for everything, straight commits, or a stack of branches (see _Git flow_).
...
Spawn a **fresh read-only subagent** and explicitly instruct it to load **`$reviewer`** before reviewing. Tell it that its scope is inspection only: it may read code, inspect git, run non-mutating checks, and open provided attachments, but it must not edit files, commit, or call board-mutating tools. The subagent reports findings; **you** dispose of them. You have board access, so curate into its prompt everything it needs:
